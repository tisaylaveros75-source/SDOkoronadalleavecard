...
// ── Collect printable content from all NT card sections ──────
function getPageContent(): string {
  // Gather the profile card
  const profileCard = document.querySelector<HTMLElement>('#ntCard');
  // Gather the leave table card(s) — there may be multiple era sections
  const tableCards = Array.from(document.querySelectorAll<HTMLElement>('#ntTblCard, .era-new-section, [id^="ntTblCard"]'));
  ...
  if (profileCard) {
    ...
    // Remove the form if it's inside
    c.querySelectorAll<HTMLElement>('#ntFrm').forEach(el => el.remove());
  ...
  if (tableCards.length > 0) {
    tableCards.forEach(card => {
      ...
      c.querySelectorAll<HTMLElement>('.no-print').forEach(el => el.remove());
      c.querySelectorAll<HTMLElement>('button').forEach(el => el.remove());
      // Force era-old-body visible for print
      c.querySelectorAll<HTMLElement>('.era-old-toggle').forEach(el => el.remove());
      c.querySelectorAll<HTMLElement>('.era-old-body').forEach(el => {
  ...
  // Final fallback: grab the whole page container
  if (!profileCard && tableCards.length === 0) {
    const fallback = document.querySelector<HTMLElement>('[id*="ntCard"], [id*="ntTbl"], .page');
    if (fallback) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (!content || content.trim() === '') {
        alert('No content found. Please wait for the page to finish loading.');
        return;
      }

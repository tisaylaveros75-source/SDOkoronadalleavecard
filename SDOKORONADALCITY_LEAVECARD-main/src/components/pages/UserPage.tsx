...
// Same base64 logo used in NTCardPage — no network request needed
...
// ── Shared print styles (identical to NTCardPage) ─────────────
const PRINT_STYLES = `
...
// ── Collect page content from the user card ───────────────────
function getCardContent(): string {
  // Try the specific user cards first, then fall back to any visible card
  const selectors = ['#userProfileCard', '#userTableCard', '.page.on'];
  
  // Collect all user-visible cards
  ...
  if (profileCard || tableCard) {
    ...
    // Fallback: grab the page container
